...
const PROJECT_X_URL = 'https://x.com/Binarahood';
...
const navGroups: NavGroup[] = [
  { title: 'Overview', items: [
    ...
    { label: 'New Tokens', href: '/new-tokens', icon: 'SparklesIcon' },
  ] },
...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const statusColor = chainStatus.status === 'live' ? 'bg-positive-subtle border-positive text-positive' : chainStatus.status === 'error' ? 'bg-destructive/10 border-destructive text-destructive' : chainStatus.status === 'stale' ? 'bg-warning-subtle border-warning text-warning' : 'bg-muted/40 border-border text-muted-foreground';
  const dotColor = chainStatus.status === 'live' ? 'live-dot' : chainStatus.status === 'error' ? 'w-2 h-2 rounded-full bg-destructive flex-shrink-0' : chainStatus.status === 'stale' ? 'w-2 h-2 rounded-full bg-warning flex-shrink-0' : 'w-2 h-2 rounded-full bg-muted-foreground animate-pulse flex-shrink-0';
  ...
  return <aside className={`fixed left-0 top-0 h-full z-40 flex flex-col bg-card border-r border-border transition-all duration-300 ease-in-out ${collapsed ? 'w-16' : 'w-60'}`}>
    ...
    <nav className="flex-1 overflow-y-auto py-3 px-2">
      {navGroups.map((group) => <div key={`group-${group.title}`} className="mb-4">
        {!collapsed && <p className="px-2 mb-1 text-xs font-semibold tracking-widest uppercase text-muted-foreground/60">{group.title}</p>}
        {group.items.map((item) => { const active = isActive(item.href); return <Link key={`nav-${item.href}`} href={item.href} title={collapsed ? item.label : undefined} className={`relative flex items-center gap-3 px-2 py-2 rounded-lg mb-0.5 transition-all duration-150 group ${active ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/60'} ${collapsed ? 'justify-center' : ''}`}>
          <Icon name={item.icon as Parameters<typeof Icon>[0]['name']} size={18} className={active ? 'text-primary' : ''} />
          {!collapsed && <><span className="text-sm font-medium flex-1">{item.label}</span>{item.isDemo && <span className="text-xs px-1.5 py-0.5 rounded bg-muted text-muted-foreground/60 font-medium">Soon</span>}{item.badge !== undefined && !item.isDemo && <span className="text-xs px-1.5 py-0.5 rounded-full bg-primary/20 text-primary font-semibold min-w-[20px] text-center">{item.badge}</span>}</>}
    ...
    {!collapsed && <div className="border-t border-border p-3 space-y-2"><p className="px-1 text-[10px] font-semibold tracking-widest uppercase text-muted-foreground/50">Project</p><a id="binara-x-link" data-project-link="x" href={PROJECT_X_URL} target="_blank" rel="noopener noreferrer" aria-label="Open Binara on X" title={PROJECT_X_URL} className="flex items-center gap-2 px-2 py-2 rounded-lg text-xs transition-colors text-muted-foreground hover:text-foreground hover:bg-muted/60"><span className="w-5 text-center text-sm font-semibold">𝕏</span><span className="flex-1 font-medium">X / Twitter</span><span className="text-[10px]">Open</span></a><div className="flex items-center gap-2 px-2 py-2 rounded-lg bg-muted/30 border border-border/60"><Icon name="DocumentDuplicateIcon" size={15} className="text-muted-foreground/70" /><div className="min-w-0 flex-1"><p className="text-[10px] text-muted-foreground/60 uppercase tracking-wide">Contract</p><p className="text-[11px] font-mono text-muted-foreground truncate">{CONTRACT_ADDRESS || 'Not launched'}</p></div></div></div>}
