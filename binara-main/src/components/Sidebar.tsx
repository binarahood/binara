...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return <aside className={`fixed left-0 top-0 h-full z-40 flex flex-col bg-card border-r border-border transition-all duration-300 ease-in-out ${collapsed ? 'w-16' : 'w-60'}`}>
    ...
    {!collapsed && <div className="border-t border-border p-3 space-y-2"><p className="px-1 text-[10px] font-semibold tracking-widest uppercase text-muted-foreground/50">Project</p><a href={PROJECT_X_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-2 py-2 rounded-lg text-xs transition-colors text-muted-foreground hover:text-foreground hover:bg-muted/60"><span className="w-5 text-center text-sm font-semibold">𝕏</span><span className="flex-1 font-medium">X / Twitter</span><span className="text-[10px]">↗</span></a><div className="flex items-center gap-2 px-2 py-2 rounded-lg bg-muted/30 border border-border/60"><Icon name="DocumentDuplicateIcon" size={15} className="text-muted-foreground/70" /><div className="min-w-0 flex-1"><p className="text-[10px] text-muted-foreground/60 uppercase tracking-wide">Contract</p><p className="text-[11px] font-mono text-muted-foreground truncate">{CONTRACT_ADDRESS || 'Not launched'}</p></div></div></div>}
