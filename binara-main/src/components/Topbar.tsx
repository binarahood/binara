...
export default function Topbar() {
  ...
  return <>
    <header className="h-16 flex items-center justify-between px-6 border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-30">
      <div className="flex items-center gap-2 text-sm"><span className="text-muted-foreground">BINARA</span><Icon name="ChevronRightIcon" size={14} className="text-muted-foreground/50" /><span className="text-foreground font-medium">Analytics Terminal</span><span className="ml-2"><DataStatusBadge /></span></div>
    ...
    {walletChooserOpen && <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-0 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="wallet-chooser-title" data-wallet-chooser-version={MOBILE_WALLET_CHOOSER_VERSION} onMouseDown={event => { if (event.target === event.currentTarget) setWalletChooserOpen(false); }}><div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl border border-border bg-card shadow-2xl overflow-hidden"><div className="px-5 py-4 border-b border-border flex items-center justify-between"><div><h2 id="wallet-chooser-title" className="text-lg font-semibold text-foreground">Connect Wallet</h2><p className="text-sm text-muted-foreground mt-1">Choose a supported EVM wallet.</p></div><button type="button" onClick={() => setWalletChooserOpen(false)} className="w-9 h-9 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" aria-label="Close wallet chooser"><Icon name="XMarkIcon" size={20} /></button></div><div className="p-4 space-y-2.5"><WalletOption icon="🦊" name="MetaMask" description="Open Binara in MetaMask mobile" onClick={() => openMobileWallet(METAMASK_DAPP_URL)} /><WalletOption icon="👻" name="Phantom" description="Open Binara in Phantom mobile" onClick={() => openMobileWallet(PHANTOM_DAPP_URL)} /><WalletOption icon="▣" name="Coinbase Wallet" description="Open Binara in Coinbase Wallet" onClick={connectCoinbase} /></div><div className="px-5 pb-5"><p className="text-[11px] leading-relaxed text-muted-foreground/70 text-center">MetaMask, Phantom and Coinbase Wallet are supported on mobile. After opening a wallet app, Binara will use its EVM connection on Robinhood Chain.</p></div></div></div>}
  </>;
}
