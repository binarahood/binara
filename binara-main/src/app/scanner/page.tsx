...
/**
 * BINARA V1 Opportunity Scanner
 *
 * This page intentionally does NOT invent a risk score, APR, holder count,
 * LP lock state, or profitability prediction. Ranking is based only on live
 * fields already verified by the pool API.
 */
function opportunityScore(pool: LivePool): number | null {
  ...
  // Relative score, not a profitability prediction:
  // 40% volume/TVL, 30% 24h volume, 20% TVL, 10% swap activity.
  return Math.round(
    Math.min(100, (pool.volumeToTVL! / 10) * 40) +
    Math.min(30, Math.log10(pool.volume24h! + 1) * 5) +
    Math.min(20, Math.log10(pool.tvl! + 1) * 3) +
    Math.min(10, pool.swapCount24h! / 100),
  );
}
      ...
      <div className="space-y-5 animate-fade-in">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Live Robinhood Chain pool opportunities — verified inputs only</p>
          </div>
          <div className="flex items-center gap-2">
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search pair or address" className="input-field h-9 w-56 text-sm" />
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border ${error ? 'bg-destructive/10 border-destructive' : isLoading ? 'bg-muted/40 border-border' : 'bg-positive-subtle border-positive/30'}`}>
              <span className={error ? 'w-2 h-2 rounded-full bg-destructive' : isLoading ? 'w-2 h-2 rounded-full bg-muted-foreground animate-pulse' : 'live-dot'} />
              <span className={`text-xs font-semibold ${error ? 'text-destructive' : isLoading ? 'text-muted-foreground' : 'text-positive'}`}>{error ? 'DATA ERROR' : isLoading ? 'LOADING' : `LIVE${secondsAgo !== null ? ` • ${secondsAgo}s` : ''}`}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex flex-wrap items-end gap-4">
          <label className="text-xs text-muted-foreground">Minimum TVL<input type="number" min="0" value={minTVL} onChange={(e) => setMinTVL(Number(e.target.value) || 0)} className="input-field mt-1 h-9 w-40" /></label>
          <label className="text-xs text-muted-foreground">Minimum 24h Volume<input type="number" min="0" value={minVolume} onChange={(e) => setMinVolume(Number(e.target.value) || 0)} className="input-field mt-1 h-9 w-40" /></label>
          <button onClick={() => { setMinTVL(0); setMinVolume(0); setSearch(''); }} className="btn-ghost h-9">Reset</button>
        </div>

        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="p-4 border-b border-border flex items-center justify-between"><div><h2 className="text-sm font-semibold">Ranked Live Opportunities</h2><p className="text-xs text-muted-foreground mt-0.5">The score ranks observed market activity; it is not a profit guarantee.</p></div><span className="text-xs text-muted-foreground">{rows.length} pools</span></div>
