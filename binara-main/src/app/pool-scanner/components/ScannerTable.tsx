    ...
    <div className="overflow-x-auto">
        ...
        <tbody>
          {sorted.map((pool, idx) => (
            <tr
              ...
              className={`
                border-b border-border/50 cursor-pointer transition-colors duration-100 group
                ${selectedId === pool.id ? 'bg-primary/5 border-primary/20' : 'hover:bg-muted/30'}
              `}
            >
              ...
              <td className="table-row-cell">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Step {pool.binStep} · {pool.protocol}</p>
                  </div>
                </div>
              </td>
              ...
              <td className="table-row-cell">
                <span className={`font-mono-nums font-semibold ${
                  pool.volumeToTVL > 5 ? 'text-positive' : pool.volumeToTVL > 2 ? 'text-foreground' : 'text-muted-foreground'
                }`}>
              ...
              <td className="table-row-cell">
                <span className={`font-mono-nums ${
                  pool.volatility > 8 ? 'text-negative' : pool.volatility > 4 ? 'text-warning' : 'text-muted-foreground'
                }`}>
                  {pool.volatility.toFixed(1)}%
                </span>
              </td>
              <td className="table-row-cell">
                {pool.timeInRange !== null ? (
                  <div className="flex items-center gap-1.5">
                    <div className="w-12 h-1.5 rounded-full bg-muted overflow-hidden">
                      <div
                        className={`h-full rounded-full ${
                          (pool.timeInRange ?? 0) >= 85 ? 'bg-positive' : (pool.timeInRange ?? 0) >= 70 ? 'bg-warning' : 'bg-negative'
                        }`}
                        style={{ width: `${pool.timeInRange ?? 0}%` }}
                      />
                    </div>
              ...
              <td className="table-row-cell">
                <span className={`font-mono-nums font-semibold text-xs ${
                  (pool.estimatedAPR ?? 0) > 100 ? 'text-positive' : (pool.estimatedAPR ?? 0) > 50 ? 'text-info' : 'text-muted-foreground'
                }`}>
              ...
              <td className="table-row-cell">
                <div className="flex items-center gap-1.5">
                  <div className="w-10 h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
                        pool.analyticsScore >= 80 ? 'bg-positive' : pool.analyticsScore >= 60 ? 'bg-warning' : 'bg-negative'
                      }`}
                      style={{ width: `${pool.analyticsScore}%` }}
                    />
                  </div>
                  <span className={`text-xs font-mono-nums font-bold ${
                    pool.analyticsScore >= 80 ? 'text-positive' : pool.analyticsScore >= 60 ? 'text-warning' : 'text-negative'
                  }`}>
                    {pool.analyticsScore}
                  </span>
                </div>
              ...
              <td className="table-row-cell">
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                  <button
                    onClick={(e) => { e.stopPropagation(); onSelect(pool); }}
      ...
      <div className="flex items-center justify-between px-3 py-2 border-t border-border">
        <p className="text-xs text-muted-foreground/60">
          * APR estimates based on recent fee activity. Not guaranteed.
        </p>
        <p className="text-xs text-muted-foreground font-mono-nums">
          {pools.length} pool{pools.length !== 1 ? 's' : ''} shown
        </p>
      </div>
    </div>
  );
