...
function computeDashboardMetrics(pools: LivePool[]): DashboardMetrics {
  ...
  const totalTVL = pools.reduce((s, p) => s + (p.tvl ?? 0), 0); const volume24h = pools.reduce((s, p) => s + (p.volume24h ?? 0), 0); const weightedFee = pools.reduce((s, p) => s + p.fee * (p.tvl ?? 0), 0); const avgFeeTier = totalTVL > 0 ? weightedFee / totalTVL : 0;
  const poolsWithVolTVL = pools.filter((p) => p.volumeToTVL !== null); const highestVolTVLPool = poolsWithVolTVL.length ? poolsWithVolTVL.reduce((best, p) => p.volumeToTVL! > best.volumeToTVL! ? p : best, poolsWithVolTVL[0]) : null;
  const poolsWithAPR = pools.filter((p) => p.estimatedAPR !== null); const bestFeePool = poolsWithAPR.length ? poolsWithAPR.reduce((best, p) => p.estimatedAPR! > best.estimatedAPR! ? p : best, poolsWithAPR[0]) : null;
  const poolsWithPriceChange = pools.filter((p) => p.priceChange24h !== null); const mostVolatile = poolsWithPriceChange.length ? poolsWithPriceChange.reduce((best, p) => Math.abs(p.priceChange24h!) > Math.abs(best.priceChange24h!) ? p : best, poolsWithPriceChange[0]) : null;
...
const STALE_THRESHOLD_MS = 60_000; const STATUS_INTERVAL_MS = 15_000; const POOL_INTERVAL_MS = 30_000;
...
function mergePools(basePools: LivePool[], enrichedPools: LivePool[]): LivePool[] {
  ...
  for (const pool of enrichedPools) for (const key of [pool.address, pool.id].filter(Boolean).map((value) => value.toLowerCase())) enrichedByAddress.set(key, pool);
  return basePools.map((base) => { const enriched = enrichedByAddress.get(base.address.toLowerCase()) ?? enrichedByAddress.get(base.id.toLowerCase()); return enriched ? { ...base, ...enriched, id: base.id, address: base.address } : base; });
}

export function usePoolsData(intervalMs = POOL_INTERVAL_MS, visibilityMode: PoolVisibility | 'all' = 'active') {
  const [pools, setPools] = useState<LivePool[]>([]); const [dashboardMetrics, setDashboardMetrics] = useState<DashboardMetrics>(() => computeDashboardMetrics([])); const [isLoading, setIsLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [lastUpdated, setLastUpdated] = useState<number | null>(null); const [indexerStatus, setIndexerStatus] = useState<string | null>(null);
  const basePoolsRef = useRef<LivePool[]>([]); const enrichmentCacheRef = useRef<LivePool[]>([]); const enrichmentInFlight = useRef(false);

  const publishPools = useCallback((allPools: LivePool[]) => { const classified = allPools.map((pool) => ({ ...pool, visibility: getPoolVisibility(pool) })); const visible = classified.filter((pool) => visibilityMode === 'all' || pool.visibility === visibilityMode); setPools(visible); setDashboardMetrics(computeDashboardMetrics(classified.filter((p) => p.visibility === 'active'))); }, [visibilityMode]);

  const fetchEnrichment = useCallback(async () => {
    if (enrichmentInFlight.current) return;
    enrichmentInFlight.current = true;
    try {
      const res = await fetch('/api/chain/pools/enrich', { cache: 'no-store' }); const data = await res.json();
      if (!res.ok || data.error) return;
      const enriched = (data.pools || []) as LivePool[];
      if (enriched.length === 0) return;
      enrichmentCacheRef.current = enriched;
      const merged = mergePools(basePoolsRef.current, enrichmentCacheRef.current);
    ...
    } finally { enrichmentInFlight.current = false; }
  }, [publishPools]);
  ...
  const fetchPools = useCallback(async () => {
    try {
      ...
      // Preserve the last successful enrichment when the fast base-pool snapshot refreshes.
      const merged = mergePools(rawPools, enrichmentCacheRef.current);
      basePoolsRef.current = merged;
      publishPools(merged);
