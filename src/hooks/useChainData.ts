...
export interface DashboardMetrics {
  ...
  bestFeePool: string;
  bestFeeAPR: number | null;
  bestFeeScore: number | null;
  mostVolatilePair: string;
  mostVolatileChange: number | null;
  lastSync: string;
}
...
function computeDashboardMetrics(pools: LivePool[]): DashboardMetrics {
  ...
  const highestVolTVLPool = poolsWithVolTVL.length
    ? poolsWithVolTVL.reduce((best, p) => p.volumeToTVL! > best.volumeToTVL! ? p : best, poolsWithVolTVL[0])
    : null;

  const poolsWithAPR = pools.filter((p) => p.estimatedAPR !== null);
  const bestFeePool = poolsWithAPR.length
    ? poolsWithAPR.reduce((best, p) => p.estimatedAPR! > best.estimatedAPR! ? p : best, poolsWithAPR[0])
    : null;

  const poolsWithPriceChange = pools.filter((p) => p.priceChange24h !== null);
  const mostVolatile = poolsWithPriceChange.length
    ? poolsWithPriceChange.reduce((best, p) => Math.abs(p.priceChange24h!) > Math.abs(best.priceChange24h!) ? p : best, poolsWithPriceChange[0])
    : null;
  ...
  return {
    ...
    volume24h,
    avgFeeTier,
    highestVolTVL: highestVolTVLPool?.volumeToTVL ?? null,
    highestVolTVLPair: highestVolTVLPool?.pair ?? 'N/A',
    bestFeePool: bestFeePool?.pair ?? 'N/A',
    bestFeeAPR: bestFeePool?.estimatedAPR ?? null,
    bestFeeScore: bestFeePool?.analyticsScore ?? null,
    mostVolatilePair: mostVolatile?.pair ?? 'N/A',
    mostVolatileChange: mostVolatile?.priceChange24h ?? null,
    lastSync,
  };
}
...
export function useChainStatus() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      if (!res.ok || data.status === 'error') {
        setChainStatus((prev) => ({ ...prev, status: 'error', error: data.error || 'Unable to retrieve live Robinhood Chain data.', lastUpdated: Date.now() }));
      } else {
        setChainStatus({ status: 'live', blockNumber: data.blockNumber, chainId: data.chainId, lastUpdated: Date.now(), error: null });
      }
    } catch {
      setChainStatus((prev) => ({ ...prev, status: 'error', error: 'Unable to retrieve live Robinhood Chain data.', lastUpdated: Date.now() }));
    }
  }, []);
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setChainStatus((prev) => {
        if (prev.status === 'live' && prev.lastUpdated && Date.now() - prev.lastUpdated > STALE_THRESHOLD_MS) return { ...prev, status: 'stale' };
        return prev;
      });
    }, 5000);
    return () => clearInterval(id);
