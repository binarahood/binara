...
export default function Topbar() {
  ...
  return (
    <header className="h-14 sm:h-16 flex items-center justify-between gap-2 px-3 sm:px-6 border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-30 min-w-0">
      ...
      {walletChooserOpen && <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Choose wallet">
        <div className="w-full max-w-md rounded-2xl bg-card border border-border shadow-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between"><div><h2 className="text-sm font-semibold text-foreground">Connect Wallet</h2><p className="text-xs text-muted-foreground mt-1">Choose an installed EIP-6963 wallet.</p></div><button onClick={() => setWalletChooserOpen(false)} className="btn-ghost p-2"><Icon name="XMarkIcon" size={16} /></button></div>
