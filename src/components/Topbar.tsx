...
function WalletChooser({ open, wallets, isConnecting, chooseWallet, close }: { open: boolean; wallets: WalletProviderInfo[]; isConnecting: boolean; chooseWallet: (wallet: WalletProviderInfo) => void; close: () => void; }) {
  if (!open || typeof document === 'undefined') return null;
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm overflow-y-auto" role="dialog" aria-modal="true" aria-label="Choose wallet">
      <div className="w-full max-w-md max-h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-2rem)] rounded-2xl bg-card border border-border shadow-2xl flex flex-col overflow-hidden">
        <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-border flex items-center justify-between flex-shrink-0">
          <div className="min-w-0"><h2 className="text-sm font-semibold text-foreground">Connect Wallet</h2><p className="text-xs text-muted-foreground mt-1 truncate">Choose a supported EVM wallet.</p></div>
          <button onClick={close} className="btn-ghost p-2 flex-shrink-0" aria-label="Close wallet chooser"><Icon name="XMarkIcon" size={16} /></button>
        </div>
        <div className="p-3 sm:p-4 space-y-2 overflow-y-auto overscroll-contain">
          {wallets.length === 0 ? <div className="rounded-xl border border-border bg-muted/20 p-4 text-center"><Icon name="WalletIcon" size={24} className="mx-auto mb-2 text-muted-foreground" /><p className="text-sm font-medium text-foreground">No compatible wallet detected</p><p className="text-xs text-muted-foreground mt-1">Install an EIP-6963 compatible EVM wallet, then reopen this dialog.</p></div> : wallets.map((wallet) => <button key={wallet.uuid} onClick={() => chooseWallet(wallet)} disabled={isConnecting} className="w-full flex items-center gap-3 rounded-xl border border-border bg-card hover:bg-muted/40 p-3 text-left transition-colors disabled:opacity-60"><WalletIcon wallet={wallet} /><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-foreground truncate">{wallet.name}</p><p className="text-[10px] text-muted-foreground truncate">{wallet.kind === 'coinbase' ? 'Coinbase Wallet extension / mobile wallet' : (wallet.rdns ?? 'EIP-6963 provider')}</p></div><Icon name="ChevronRightIcon" size={14} className="text-muted-foreground flex-shrink-0" /></button>)}
          <div className="rounded-xl border border-dashed border-border p-3 mt-3"><p className="text-[11px] font-semibold text-muted-foreground">Supported</p><p className="text-[10px] text-muted-foreground mt-1">MetaMask, Coinbase Wallet and other compatible EVM providers are supported. TronLink is intentionally excluded.</p></div>
        </div>
      </div>
    </div>,
    document.body
...
export default function Topbar() {
  ...
  const chooseWallet = async (wallet: WalletProviderInfo) => { setWalletChooserOpen(false); await connect(wallet); };
    ...
    <>
      <header className="h-14 sm:h-16 flex items-center justify-between gap-2 px-3 sm:px-6 border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-30 min-w-0">
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
          ...
          <a href="https://x.com/Binarahood" target="_blank" rel="noopener noreferrer" aria-label="Binara on X" title="Binara on X" className="inline-flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-lg border border-border bg-muted/20 text-foreground hover:bg-muted/60 transition-colors text-base sm:text-lg font-semibold leading-none">𝕏</a>
      ...
      <WalletChooser open={walletChooserOpen} wallets={availableWallets} isConnecting={isConnecting} chooseWallet={chooseWallet} close={() => setWalletChooserOpen(false)} />
    </>
  );
}
