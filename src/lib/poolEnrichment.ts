...
import { RobinhoodSubgraphPool, toLivePool } from '@/lib/robinhoodSubgraph';
...
export async function enrichPools(sourcePools: RobinhoodSubgraphPool[]) {
  ...
  const marketPromise = fetchGeckoPoolMarketData(poolAddresses);
  const metadataPromise = fetchTokenMetadata(tokenAddresses);
  ...
  return { pools, dataQuality: {
    ...
    gmgnTokenBudget: MAX_GMGN_TOKENS, verifiedTokenMetadataCount: tokenMetadata.size,
    geckoTokenPriceCount: geckoTokenPrices.byToken.size, geckoTokenPriceBatchesSucceeded: geckoTokenPrices.batchesSucceeded,
    geckoTokenPriceBatchesAttempted: geckoTokenPrices.batchesAttempted, volumeWindowSeconds: 86_400, volumeComplete: market.complete,
    poolsDiscovered: sourcePools.length, poolsWithMarketData: market.poolsReturned,
    poolsMissingPrimaryMarketData: missingPoolAddresses.size, geckoBatchesSucceeded: market.batchesSucceeded, geckoBatchesAttempted: market.batchesAttempted,
