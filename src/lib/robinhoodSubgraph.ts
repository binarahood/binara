...
export async function getPools(limit = 500): Promise<RobinhoodSubgraphPool[]> {
  ...
  const data = await query<{ DLMMPool?: RobinhoodSubgraphPool[] }>(`query GetPools($chainId: Int!, $limit: Int!) {
    DLMMPool(where: { chainId: { _eq: $chainId } } limit: $limit) { id tokenX tokenY binStep activeId reserveX reserveY }
...
export function toLivePool(pool: RobinhoodSubgraphPool) {
  ...
  return {
    ...
    tvl: null, reserveX: pool.reserveX || '0', reserveY: pool.reserveY || '0', volume1h: null, volume6h: null,
    volume24h: null, volumeRaw24h: 0, volumeToTVL: 0, volatility: 0, analyticsScore: 35,
    riskLevel: 'MEDIUM' as const, estimatedAPR: null, timeInRange: null, swapCount24h: 0, swapCount1h: 0,
