/** Live Robinhood Chain pool data source. */
...
export interface RobinhoodSubgraphPool {
  ...
  totalValueLockedUSD?: string | null; volumeUSD?: string | null; feesUSD?: string | null; txCount?: number;
  createdAtBlockNumber?: number; createdAtTimestamp?: number; isAlive?: boolean;
}
...
async function query<T>(body: string, variables: Record<string, unknown> = {}): Promise<T> {
  ...
  try {
    ...
    if (!result.data) throw new Error('Subgraph returned no data'); return result.data;
...
export async function getPools(limit = 500): Promise<RobinhoodSubgraphPool[]> {
  ...
  const data = await query<{ DLMMPool?: RobinhoodSubgraphPool[] }>(`query GetPools($chainId: Int!, $limit: Int!) {
    DLMMPool(where: { chainId: { _eq: $chainId } } limit: $limit) {
      id tokenX tokenY binStep activeId reserveX reserveY totalValueLockedUSD volumeUSD feesUSD txCount createdAtBlockNumber createdAtTimestamp isAlive
    }
...
export function toLivePool(pool: RobinhoodSubgraphPool) {
  ...
  return {
    id: pool.id, address: pool.id, pair: `${shortAddress(tokenX)}/${shortAddress(tokenY)}`, tokenA: shortAddress(tokenX), tokenB: shortAddress(tokenY), tokenAAddress: tokenX, tokenBAddress: tokenY,
