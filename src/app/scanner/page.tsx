...
function Metric({ label, value }: { label: string; value: string }) { return <div><p className="data-label mb-0.5">{label}</p><p className="text-sm font-semibold font-mono-nums">{value}</p></div>; }
function displayPair(pool: LivePool) { return `${pool.tokenAName || pool.tokenA || 'Unknown'}/${pool.tokenBName || pool.tokenB || 'Unknown'}`; }
...
export default function ScannerPage() {
  ...
  const rows = useMemo(() => {
    ...
    return pools.filter((p) => p.status === 'active').filter((p) => (p.tvl ?? -1) >= minTVL && (p.volume24h ?? -1) >= minVolume).filter((p) => !q || displayPair(p).toLowerCase().includes(q) || p.pair.toLowerCase().includes(q) || p.address.toLowerCase().includes(q)).map((pool) => ({ pool, score: getOpportunityScore(pool) })).filter((row): row is { pool: LivePool; score: number } => row.score !== null).sort((a, b) => b.score - a.score);
  ...
  return <AppLayout><div className="space-y-5 animate-fade-in">
    <div className="flex items-center justify-between flex-wrap gap-3"><div><h1 className="text-2xl font-bold text-foreground">Opportunity Scanner</h1><p className="text-sm text-muted-foreground mt-0.5">Live Robinhood Chain pool opportunities — verified inputs only</p></div><div className="flex items-center gap-2"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search pair or address" className="input-field h-9 w-56 text-sm" /><div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border ${error ? 'bg-destructive/10 border-destructive' : isLoading ? 'bg-muted/40 border-border' : 'bg-positive-subtle border-positive/30'}`}><span className={error ? 'w-2 h-2 rounded-full bg-destructive' : isLoading ? 'w-2 h-2 rounded-full bg-muted-foreground animate-pulse' : 'live-dot'} /><span className={`text-xs font-semibold ${error ? 'text-destructive' : isLoading ? 'text-muted-foreground' : 'text-positive'}`}>{error ? 'DATA ERROR' : isLoading ? 'LOADING' : `LIVE${secondsAgo !== null ? ` • ${secondsAgo}s` : ''}`}</span></div></div></div>
