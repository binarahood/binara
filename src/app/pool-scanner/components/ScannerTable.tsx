  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        ...
        <tbody>
          {sorted.map((pool, idx) => {
            const detailHref = pool.address ? `/pool-detail?address=${encodeURIComponent(pool.address)}` : null;
            return (
              <tr
                key={pool.id}
              ...
              >
                ...
                <td className="table-row-cell">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    {detailHref && (
                      <Link href={detailHref} onClick={(e) => e.stopPropagation()} className="btn-ghost text-xs px-2 py-1" title="Open pool detail">
